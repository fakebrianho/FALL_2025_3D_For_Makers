...
const tl = gsap.timeline({
	paused: true, // for now, can autoStart if you want
...
function init() {
	...
	gsap.to(camState, {
		...
		onUpdate: () => {
			...
			const lookPos = meshes.track.trackCurve.getPoint(
				Math.min(t + 0.01, 1)
			)
...
function updateCamera(progress) {
	const t = progress
	const pos = meshes.track.trackCurve.getPoint(t)
	const look = meshes.track.trackCurve.getPoint(Math.min(t + 0.01, 1))
...
function animate() {
	...
	const y = virtualScroll.update()
	const progress = scroll.y / scroll.max
	updateCamera(progress)
	ScrollTrigger.update() // forces GSAP to read the new scroll
